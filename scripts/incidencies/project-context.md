...
- Purpose: rows with autonumeric `id`, `student_id`, parent `row_id`, `date`, `student`, blank/future `comment`, `teacher_email` for the last teacher who saved the comment, plus `active` and `inactive_comment` for active/inactive assignment state.
...
Planned `Sessions dimarts`: page over `study_group_teachers` and `study_group_students`. Datepicker defaults to today if today matches `config.Dia_Grup_Estudi`, otherwise next configured weekday. After that default is established, selecting another day shows that exact date so moved sessions remain accessible. Left/right arrows beside the datepicker move to previous/next week. Show teacher list for selected `data`. Show active `study_group_students` rows for selected `date` with columns `id`, `date`, `student`, editable `comment`, and a Save button that updates changed comments in existing rows. Blank `active` values are treated as active; false/FALSE rows are hidden. Each row reveals a red X and a three-dot options menu on hover/focus. The X cancels only the clicked session after collecting a required `Motiu`, setting that row's `active` to FALSE and `inactive_comment` to the reason. `Moure sessió` opens a Catalan Monday-first date picker and changes only that row's `date`.

Planned `Equip 3R`: full-month weekday-only calendar page over `3r_project`, loaded from the left menu. It excludes Saturday/Sunday, has Catalan month navigation (`Gener` through `Desembre`) with left/right arrows, and defaults to the current month. Each weekday cell shows date, configured teacher, assigned student, and an `aprofitament` combobox with `Fet amb aprofitament` / `Absent o no aprofitat`. Every assignment has a top-right X that confirms and deletes only that `3r_project` row, plus a vertical three-dot menu whose `Moure sessió` action changes only the row's date through a Catalan Monday-first weekday picker and opens the destination month. Floating Save updates changed `aprofitament` values only.
